...
class CarrisApiClient:
    ...
    def token_is_expired(self) -> bool:
        ...
        return datetime.now(UTC) >= (self._token_expires_at - TOKEN_REFRESH_BUFFER)
    ...
    async def refresh_token(self) -> str:
        ...
        async with (
            ...
            _LOGGER.debug(
                "Carris token refreshed, expires at %s", self._token_expires_at.isoformat()
            )
            return self._token
    ...
    async def _request_with_retry(self, url: str, headers: dict[str, str]) -> Any:
            ...
            try:
                async with (
                    ...
                    if response.status == 429:
                        ...
                        _LOGGER.warning("Rate limited, waiting %s seconds before retry", wait_time)
                ...
                if attempt < MAX_RETRIES:
                    _LOGGER.warning(
                        "Timeout on attempt %d, retrying in %s seconds", attempt + 1, delay
                    )
                    await asyncio.sleep(delay)
                    delay = min(delay * 2, MAX_RETRY_DELAY)
                else:
                ...
                if attempt < MAX_RETRIES:
                    ...
                    _LOGGER.error("Connection failed after %d attempts: %s", MAX_RETRIES + 1, err)
                    raise
    ...
    async def get_buses_for_route(
        ...
        matching_buses = [
            bus for bus in snapshot if isinstance(bus, dict) and bus.get("route") == route_number
        ]

        # Filter by direction if specified
        if direction is not None:
            matching_buses = [bus for bus in matching_buses if bus.get("direction") == direction]
    ...
    async def get_direction_for_stop(self, route_number: str, stop_id: int) -> int | None:
        ...
        _LOGGER.warning(
            "Could not determine direction for route %s at stop %d", route_number, stop_id
        )
        return None
